...
State.prototype.callForEachNext = function (stack,args,done,index=0) {
  ...
  try{
    const _this=this;
    action.callFunction(...action.args.concat(args,[(error)=>_this.callForEachNext(stack,args,done,++index)]))
  } catch(ex){
    error++;
...
State.prototype.setCheck = function (callFunction,interval) {
  if(interval) this.checkInterval=interval;
  else this.checkInterval??=1000;
  this.setCheckOn();
...
State.prototype.testConnected = function () {
  if (this.available==false) throw Error("Disconnected")
  if (this.transitioning.down==true) throw Error("Disconnecting")
  if (this.transitioning.up==true) throw Error("Connecting")
  return this;
}
State.prototype.testUp = State.prototype.testConnected;
State.prototype.testDisconnected = function () {
  if (this.available==true) throw Error("Connected")
  if (this.transitioning.down==true) throw Error("Disconnecting")
  if (this.transitioning.up==true) throw Error("Connecting")
  return this;
}
...
State.prototype.whenUp = function (callFunction,...args) {
  ...
  if(this.available==true && q.length==0) {
    ...
    if(this.upActionCall) {
      try{
        if(this.upOnUpQDepth==null || this.upOnUpQDepth<q.length) return this;
        this.setUp()
      } catch(ex) {
